...
void main() {
    ...
    // 4. Task: temperature -- modulated by latitude and elevation
    float latFactor = smoothstep(-500.0, 50.0, coord.y);
    vec3 tempColor = vec3(1.0, 0.0, 0.0);
    tempColor = mix(tempColor, color, latFactor);
    tempColor = mix(tempColor, color, deepSea);
    tempColor = mix(tempColor, color, 5.0 * mountainFactor);
    color = tempColor;
