...
Hit intersect(Ray r) {
    ...
    // new sphere
    sphere(h, r, vec4(-1.2, -1.3, -24.0, 0.7),
        Material(vec3(0.6, 0.2, 0.7), vec3(0.8), false, false));
    circle(h, r, -2.0, 50.0,
...
vec3 illuminate(vec3 lightPosition, vec3 pos, vec3 wo, Hit h) {
    // shadow simulation
    Ray light = Ray(pos, lightPosition - pos);
...
vec3 calcLighting(vec3 pos, vec3 wo, Hit h) {
    ...
    // New point light
    color += 15.0 * illuminate(vec3(0.0, -5.0, 0.0), pos, wo, h);
    return color;
}
